...
public class UsersController : BaseController
{
    ...
    [Authorize]
    [HttpPost("skills/{skillId}/endorse")]
    public async Task<IActionResult> EndorseSkill(int skillId, CancellationToken cancellationToken)
    {
        var userId = CurrentUserId;
        if (userId == null)
            return Unauthorized(Result.Failure("Authentication required.", ResultStatus.Unauthorized));

        var result = await _userProfileService.EndorseSkillAsync(userId.Value, skillId, cancellationToken);
        return ToActionResult(result);
    }

    [Authorize]
    [HttpPost("polls/{pollId}/vote/{optionId}")]
    public async Task<IActionResult> VotePoll(int pollId, int optionId, CancellationToken cancellationToken)
    {
        var userId = CurrentUserId;
        if (userId == null)
            return Unauthorized(Result.Failure("Authentication required.", ResultStatus.Unauthorized));

        var result = await _userProfileService.VotePollAsync(userId.Value, pollId, optionId, cancellationToken);
        return ToActionResult(result);
    }
